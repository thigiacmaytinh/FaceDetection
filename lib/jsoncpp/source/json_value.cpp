...
#ifdef USE_JSON_MOVE_OPERATOR
Value::Value(Value &&other)
	: type_(nullValue), comments_(0), allocated_(0)
{
	swap(other);
	comments_ = other.comments_;
	other.comments_ = 0;
}
#endif // USE_JSON_MOVE_OPERATOR
...
{
   Value temp( other );
   //cs: not sure why but this generates a leak in value_.map_, why the swapping of values?
   swap( temp );
   return *this;
}
